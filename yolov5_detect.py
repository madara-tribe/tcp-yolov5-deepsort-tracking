...
from tcp_utils.tcp_img_sender import create_bts_socket_data, tcp_sender
import socket
import argparse
...
class Yolov5_Detction:
    def __init__(self, host='192.xxx.xx.xxx', port=8485):
        ...
        self.create_tcp_socket(host, port)

    def create_tcp_socket(self, host, port):
    ...
    def yolov5_detection(self, opt):
        ...
        for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                data, size = create_bts_socket_data(im_rgb)
                #self.client_socket.sendall(pack(">L", size) + data)
                tcp_sender(self.client_socket, size, data, pack_format=">L")
